...
import { goal, type Runner, type State } from "./state";

export const stepRunnerLogic = (state: State) => {
	for (const runner of state.runners) {
		if (runner.goal === goal.idle) {
			if (runner.randomTargetCount === 0) {
				...
				runner.animationIndex = sprite.jump;
...
export const createPhysicStepper = (n: number) => {
	// acceleration array
	const accelerations = Array.from({ length: n }, () => new Float32Array(2));

	const grid = createSpacialPartitioningGrid({
		width: 32,
		height: 8,
		capacity: Math.min(1000, Math.floor(n / 6)), // let's assume not all runners will end up in the same cell
	});
 ...
 *
 * Notes: if there is more entity in the cell than the capacity planned, it does not crash, the surplus entities are ignored.
 */
